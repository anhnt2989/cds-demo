...
const PALETTES: Record<string, readonly [string, string, string]> = {
  ai:          ['#1A0845', '#4A2BA3', '#8B5CF6'],
  government:  ['#001A4D', '#003399', '#007AFF'],
  education:   ['#003D1A', '#0D7A35', '#30D158'],
  healthcare:  ['#4D0000', '#CC1400', '#FF453A'],
  finance:     ['#4D2800', '#CC6600', '#FF9F0A'],
  smartcity:   ['#002B4D', '#005C99', '#64D2FF'],
  agriculture: ['#00332F', '#00736B', '#5AC8FA'],
  security:    ['#0A0A0A', '#1C1C1E', '#48484A'],
  tourism:     ['#4D001A', '#CC0033', '#FF375F'],
  enterprise:  ['#1C1C1E', '#3A3A3C', '#636366'],
};

export function FeaturedCard({ product, onPress }: Props) {
  const cfg     = CATEGORY_CONFIG[product.cat];
  const palette = PALETTES[product.cat] ?? PALETTES.enterprise;

  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [ss.card, pressed && ss.pressed]}>
      <LinearGradient
        colors={palette}
        start={{ x: 0.2, y: 0 }}
        end={{ x: 1, y: 1 }}
      ...
      <View style={ss.inner}>
        ...
        <View style={ss.footer}>
          <View style={ss.devChip}>
            <Text style={ss.devTxt} numberOfLines={1}>{product.dev || cfg.label}</Text>
          </View>
...
const ss = StyleSheet.create({
  card: {
    width: '100%', height: 400, borderRadius: 22, overflow: 'hidden',
    shadowColor: '#000', shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.22, shadowRadius: 18, elevation: 10,
  },
  pressed: { transform: [{ scale: 0.97 }], opacity: 0.92 },
  ring: { position: 'absolute', width: 320, height: 320, borderRadius: 160, borderWidth: 1, right: -90, top: -80 },
  ...
  catTagTxt: { fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.75)', letterSpacing: 0.6 },
  goldTag: {
  ...
  devChip: { flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7 },

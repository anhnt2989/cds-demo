...
export function AppRow({ product, onPress, divider = true }: Props) {
  ...
  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [ss.row, pressed && ss.pressed]}>
      <View style={ss.icon}>
        <VIcon name={cfg.icon} size={26} color={C.dark} />
      </View>
      <View style={ss.texts}>
        <Text style={ss.name} numberOfLines={1}>{product.name}</Text>
...
const ss = StyleSheet.create({
  ...
  icon: {
    ...
    borderWidth: StyleSheet.hairlineWidth, borderColor: C.border, flexShrink: 0,
  },
  texts: { flex: 1, gap: 2 },
  ...
  divider: {
    position: 'absolute', bottom: 0, left: 84, right: 16,

...
import {
  FlatList, Keyboard, Pressable, ScrollView, StatusBar,
...
import { ALL_CATEGORIES, CATEGORY_CONFIG, PRODUCTS, searchProducts, type Category } from '@/data/products';

const TRENDING = [
  'AI', 'GreenNode', 'Viettel', 'Chatbot', 'Giáo dục', 'Bảo hiểm',
  'Smart City', 'An ninh', 'CMC', 'VNPT', 'Bảo mật', 'ERP',
];
...
export default function SearchScreen() {
  ...
  const cancel = useCallback(() => {
    setQuery(''); setActiveCat(undefined); setFocused(false);
    Keyboard.dismiss(); ref.current?.blur();
  }, []);
    ...
    <SafeAreaView style={ss.safe} edges={['top']}>
      ...
      <ScrollView
        ...
        {ALL_CATEGORIES.map(cat => {
          ...
          return (
            <Pressable key={cat} onPress={() => setActiveCat(on ? undefined : cat)}
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <AppRow
              ...
              onPress={() => { Keyboard.dismiss(); router.push(`/product/${item.id}`); }}
            />
          )}
        />
      )}
...
const ss = StyleSheet.create({
  ...
  searchRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingTop: 8, paddingBottom: 4, gap: 10 },
  bar: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#EBEBEB', borderRadius: 12, paddingHorizontal: 10, height: 38, gap: 7 },
  ...
  pill: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, paddingVertical: 6, backgroundColor: '#fff', borderRadius: 16, borderWidth: 1, borderColor: '#E0E0E0' },
  ...
  sugTitle: { fontSize: 20, fontWeight: '800', color: '#0A0A0A', letterSpacing: -0.3, marginBottom: 12 },
  trendGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  trendChip: { backgroundColor: '#fff', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1, borderColor: '#E0E0E0' },
  trendTxt: { fontSize: 14, color: '#1C1C1E' },

  catRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 14, padding: 12, marginBottom: 8, gap: 12, borderWidth: StyleSheet.hairlineWidth, borderColor: '#E0E0E0' },
  catIconBox: { width: 44, height: 44, borderRadius: 12, backgroundColor: '#F2F2F7', alignItems: 'center', justifyContent: 'center' },
  catTexts: { flex: 1 },
